...
function RankMedal({ rank }) {
  ...
  if (rank === 2) return <span className="text-[11px] font-bold text-slate-600">3</span>
  return <span className="text-[11px] font-mono text-slate-700">{rank + 1}</span>
}
...
export default function AgentTable({ agents }) {
  ...
  const th = (col, label, align = 'left') => {
    ...
    return (
      <th
        className={`px-4 py-3 text-${align} text-[10px] text-slate-600 font-semibold uppercase tracking-wider cursor-pointer hover:text-slate-400 transition select-none whitespace-nowrap`}
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-[#0d1424] overflow-hidden h-full">
      <div className="px-5 py-4 border-b border-white/[0.06]">
        ...
        <p className="text-[11px] text-slate-600 mt-0.5">Ranked by performance metrics</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/[0.04]">
              <th className="px-4 py-3 text-center text-[10px] text-slate-600 font-semibold uppercase tracking-wider w-12">#</th>
              {th('agentName', 'Agent')}

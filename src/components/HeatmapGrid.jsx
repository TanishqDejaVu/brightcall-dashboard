  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-[#0d1424] p-5">
      <div className="mb-4">
        ...
        <p className="text-[11px] text-slate-400 mt-0.5">Call volume intensity by day and hour</p>
      </div>
      <div className="overflow-x-auto">
        {/* Hour labels */}
        <div className="flex gap-1 mb-1.5 ml-10">
          {HOURS.map(h => (
            <div key={h} className="w-5 text-center text-[8px] text-slate-500 flex-shrink-0">
              {h % 3 === 0 ? h : ''}
            </div>
          ))}
        </div>
        ...
        {heatmap.map((row, di) => (
          <div key={di} className="flex items-center gap-1 mb-1">
            <div className="w-9 text-[9px] text-slate-400 text-right pr-1.5 flex-shrink-0 font-medium">
              {DAY_LABELS[di]}
            </div>
            {row.map((val, hi) => (
              <div

...
export default function Navbar({ dateRange, selectedAgent, onRangeChange, onAgentChange, onRefresh, loading, lastUpdated, isDark, toggleTheme }) {
  ...
  return (
    <nav
      className="sticky top-0 z-50 flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-2.5 sm:py-3.5 backdrop-blur-xl border-b gap-2 sm:gap-0"
    ...
    >
      ...
      <div className="flex items-center justify-between sm:contents">
        ...
        <img
          ...
          className="h-14 sm:h-32 w-auto object-contain dark:invert -my-1 sm:-my-8"
        />

        <div className="hidden sm:block w-px h-5 bg-slate-200 dark:bg-white/[0.07] mx-5" />

        {/* Right controls — visible on all screens, pushed right on mobile */}
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 sm:ml-auto">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none cursor-pointer pl-7 pr-6 py-1.5 rounded-lg bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-[11px] font-semibold text-slate-600 dark:text-slate-400 outline-none hover:border-blue-400 dark:hover:border-white/20 transition focus:border-blue-500/40"
            >
              {AGENTS.map(a => (
      ...
      {/* Row 2: Date tabs (scrollable on mobile) */}
      <div className="flex items-center gap-2.5 sm:mx-5 overflow-hidden">
        <div className="overflow-x-auto scrollbar-hide flex-1 sm:flex-none">
          <div className="flex items-center bg-slate-100 dark:bg-white/[0.04] p-1 rounded-lg border border-slate-200 dark:border-white/[0.06] gap-0.5 w-max sm:w-auto">
            {TABS.map(t => (
              <button
                key={t.id}
                onClick={() => handleTabClick(t.id)}
                className={`px-3 py-1 text-[11px] font-semibold rounded-md transition-all whitespace-nowrap flex-shrink-0 ${
        ...
        {showCustomPicker && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto sm:pl-3 sm:border-l sm:border-slate-200 dark:sm:border-white/[0.07] mt-1 sm:mt-0">
            <input
              type="date"
              value={customStart}
              onChange={e => setCustomStart(e.target.value)}
              className="w-full sm:w-auto px-2.5 py-1 text-xs font-medium border border-slate-300 dark:border-white/[0.08] rounded-lg bg-white dark:bg-white/[0.04] text-slate-700 dark:text-slate-300 outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 dark:[color-scheme:dark]"
            />
            ...
            <button
              ...
              className="w-full sm:w-auto px-3 py-1 text-[11px] font-semibold bg-blue-600 text-white rounded-lg hover:bg-blue-500 disabled:opacity-30 transition"
            >
              Apply
            </button>
          </div>

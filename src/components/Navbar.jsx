...
const AGENTS = [
  ...
  { id: '827909', label: 'Sana' },
  { id: '827910', label: 'Huzaifa' },
  { id: '827911', label: 'Mohammed' },
  { id: '827912', label: 'Mehak' },
]
...
export default function Navbar({ dateRange, selectedAgent, onRangeChange, onAgentChange, onRefresh, loading, lastUpdated, isDark, toggleTheme }) {
  ...
  return (
    <nav
      className="sticky top-0 z-50 flex flex-col backdrop-blur-xl border-b"
      style={{ background: 'var(--navbar-bg)', borderBottomColor: 'var(--navbar-bd)' }}
    >
      {/* Row 1: Logo | Title (dead-centre) | Controls */}
      <div className="relative flex items-center px-4 sm:px-6 py-2.5 sm:py-3">
        ...
        <img
          ...
          className="h-20 sm:h-32 w-auto object-contain dark:invert -my-3 sm:-my-8 flex-shrink-0 relative z-10"
        />

        {/* Title — absolutely centred so it ignores sidebar widths */}
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none">
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight">
            <span className="text-slate-900 dark:text-white">Tele</span><span className="bg-gradient-to-r from-blue-400 via-sky-300 to-indigo-400 bg-clip-text text-transparent">sales</span><span className="text-slate-900 dark:text-white"> Dashboard</span>
          </h1>
          <p className="text-[10px] sm:text-[11px] text-slate-400 font-medium tracking-widest uppercase mt-1 hidden sm:block">
            Every call counts. Every lead matters.
          </p>
        </div>

        {/* Right controls */}
        <div className="flex items-center gap-2 sm:gap-3 ml-auto flex-shrink-0 relative z-10">
          {/* Live indicator */}
      ...
      {/* Row 2: Date tabs + Agent filter */}
      <div
        className="flex items-center gap-3 px-4 sm:px-6 py-2 border-t overflow-x-auto scrollbar-hide"
        style={{ borderTopColor: 'var(--navbar-bd)' }}
      >
        {/* Date tabs */}
        <div className="flex items-center bg-slate-100 dark:bg-white/[0.04] p-1 rounded-lg border border-slate-200 dark:border-white/[0.06] gap-0.5 flex-shrink-0">
          {TABS.map(t => (
            <button
              key={t.id}
        ...
        <div className="w-px h-4 bg-slate-200 dark:bg-white/[0.07] flex-shrink-0" />
        ...
        {showCustomPicker && (
          <div className="flex items-center gap-2 pl-3 border-l border-slate-200 dark:border-white/[0.07] flex-shrink-0">
            <input
              type="date"
              value={customStart}
